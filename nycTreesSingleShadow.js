...
map.on('load', function() {
	...
	map.addSource('trees', { type: 'geojson', data: './data/tile987187.geojson'});

  	map.addSource('treespoly', { type: 'geojson', data: './data/tile987187polygon.geojson'});

  	map.addSource('buildings', { type: 'geojson', data: './data/tile987187buildings.geojson'});
...
//
	...
	function shadow(treeID,date) {
		...
		for (let i = 0; i < buildingPoints.length; i++) {
			...
			var buildingPointGround = [ x - (z/tanAmp*(sinAz))/84540.70  , y - (z/tanAmp*(cosAz))/111047.70 ];
	...
	function stretchHoursBar() {
		...
		var min = times.sunrise.getHours() + times.sunrise.getMinutes()/60.0 - offset/60 + 0.5 ;
		var max = times.sunset.getHours() + times.sunset.getMinutes()/60.0 - offset/60 - 0.5 ;

		var width = ((max-min)*4);
		var buffer = (100-width)/2;
		document.getElementById("hourslider").min = min;
		document.getElementById("hourslider").max = max;
		document.getElementById("hourslider").style.width = width.toString().concat('%');
		document.getElementById("hourslider").style.marginLeft = buffer.toString().concat('%');
	};
	...
	map.on('click', 'buildingfootprints', function(e) {
		...
		map.setFilter('buildingExtruded', ['in', 'bin', bin]);
...
function pointColor(point,vs,tanAmp,sinAmp,cosAz) {
	...
	var xg = x - (z/tanAmp*(sinAz))/84540.70;
    var yg = y - (z/tanAmp*(cosAz))/111047.70;
...
function htmlCountUpdate(_callback){
	...
	document.getElementById("inshadow").innerHTML = (shadedPoints.length).toString();
	document.getElementById("shadingbuilding").innerHTML = (shadingPoints.length).toString();
